...
contract SpecialRaceBasement is Ownable,IERC20,IERC20Metadata {
    ...
    function init(address tokenOwner, string memory symbols, string memory names, uint fixedSupply) external {
       require(msg.sender == address(ITokenRacesFactory(msg.sender)), "Only TokenRacesFactory"); 
...
contract SpecialRaceFactory is Ownable,ITokenRacesFactory {    
    address tokenRacesRouter;
    ...
    function startRacingToken(
        ...
        uint totalSupply,
        uint32 id) external override  returns (address raceAddress) {
        require(msg.sender == tokenRacesRouter, "Only TokenRacesRouter");
        SpecialRaceBasement race = new SpecialRaceBasement();
        race.init(msg.sender, symbol, name, totalSupply);
        currentRaceId=id;
        return address(race);
    }
